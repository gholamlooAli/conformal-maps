...
void Draw(void)
{
  ...
  draw_video_frame(callback.sourceparams, callback.displaydata);
  draw_symbology(callback.sourceparams, callback.displaydata);
		      ...
		      Displaydata_t * displaydata)
{
  ...
  static GLfloat v3[3] = { 1.00, -1.00, 0.0}; /* vertices of  */
  static GLfloat v2[3] = {-1.00, -1.00, 0.0}; /* polygon  */
  static GLfloat v1[3] = {-1.00, 1.00, 0.0};
  static GLfloat v0[3] = {1.00, 1.00, 0.0};
#else
  static GLfloat v3[3] = {-1.00, -1.00, 0.0}; /* vertices of  */
  static GLfloat v2[3] = { 1.00, -1.00, 0.0}; /* polygon  */
  static GLfloat v1[3] = { 1.00, 1.00, 0.0};
...

  {
      ...
      else
	{
	  ...
	  glBegin(GL_QUADS);
	  {	  
	    glTexCoord2fv(displaydata->t0); glVertex3fv(v0);
	    glTexCoord2fv(displaydata->t1); glVertex3fv(v1);
	    glTexCoord2fv(displaydata->t2); glVertex3fv(v2);
	    glTexCoord2fv(displaydata->t3); glVertex3fv(v3);
			...
			Displaydata_t * displaydata)
{
  ...
  char frameratestring[FPS_STRINGLEN];
  ...
  if (0 == have_last_time)
    {
      last_time = time;
      sprintf(frameratestring, "Frames/Sec: ?");
      have_last_time = 1;
      glutPostRedisplay();
    }
  else
    {
      ...
      last_time = time;
      if (0 == elapsed_millisec)
	{
	  sprintf(frameratestring, "Frames/Sec: ?");
	}
      else
	{
	  frames_sec = 1000 / elapsed_millisec; /* integer division's fine  */
	  sprintf(frameratestring, "Frames/Sec: %d", frames_sec);
	}
    }
				...
				Sourceparams_t * sourceparams)
{
  ...
  glutIdleFunc(Idle);
  setup_menu();
  glutTimerFunc(500, timer_fuction, 0);
}
